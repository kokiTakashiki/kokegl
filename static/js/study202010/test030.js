...
function init(){
	...
	for(var i = 0;  i < kokehani2.x;  i++){
		for(var j = 0;  j < kokehani2.y;  j++){
			...
			var dist = Math.sqrt((cx*cx) + (cz*cz))
			if (dist <= radius2) {
				poslist2.push(new THREE.Vector3(i/kokemitu2+kokemargin2.x,0.0,j/kokemitu2+kokemargin2.y))
			}
		}
	}
	...
	for(var i = 0;  i < kokehani3.x;  i++){
		for(var j = 0;  j < kokehani3.y;  j++){
			...
			var dist = Math.sqrt((cx*cx) + (cz*cz))
			if (dist <= radius3) {
				poslist3.push(new THREE.Vector3(i/kokemitu3+kokemargin3.x,0.0,j/kokemitu3+kokemargin3.y))
			}
		}
	}
